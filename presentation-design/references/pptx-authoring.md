...
## Raster Images in Editable PPTX

PNG/JPG/WebP допустимы и ожидаемы для содержательных иллюстраций в editable PPTX. Редактируемость PowerPoint относится к тексту, фигурам, таблицам, графикам и layout-слоям; она не требует заменять людей, одежду, предметы, сцены или учебные изображения на SVG.

Если слайду нужен узнаваемый объект, сначала используй растровый путь из `content-assets.md`: локальный файл, материалы пользователя, открытый источник или генерация через `imagegen`. Вставляй итоговый файл через `<img>`, а подписи, вопросы, словарь, стрелки и выноски держи отдельными редактируемыми слоями.
...
5. Экспортируй все слайды в PNG и пройди `slide-qa.md` с реестром `exports/slide-qa.md`:

```bash
node scripts/export_deck_png.mjs --slides slides --out exports/png
```

6. После чистого прохода `1..N` запусти PPTX-экспорт:
