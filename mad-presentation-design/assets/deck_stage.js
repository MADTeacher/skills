/**
 * <deck-stage> — web component-оболочка для HTML-слайдов
 *
 * Возможности:
 * - Canvas фиксированного размера (по умолчанию 1920×1080), auto-scale и letterbox
 * - Навигация с клавиатуры (←/→/Space/Home/End/Esc)
 * - Навигация кликом по левой/правой зоне
 * - Счетчик слайдов (текущий/всего)
 * - Сохранение текущего слайда в localStorage
 * - Заметки докладчика через postMessage (поддерживают внешний рендеринг)
 * - Hash-навигация (#slide-5 переходит к 5-му слайду)
 * - Поддержка Print-to-PDF (Cmd+P / Ctrl+P, один слайд на страницу)
 * - Автоматически добавляет data-screen-label каждому слайду
 *
 * Использование:
 *   <deck-stage>
 ...
 *
 * Заметки докладчика: добавьте в <head>
 *   <script type="application/json" id="speaker-notes">
 *   ["заметки к слайду 1", "заметки к слайду 2"]
 *   </script>
 */
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Сначала рендерим Shadow DOM (он независим от дочерних узлов и тайминга parser)
      this._render();

      // Защита: если script стоит в <head> (а не после </deck-stage>),
      // parser мог еще не обработать дочерние <section>, и querySelectorAll вернет пусто.
      // Откладываем до следующего цикла событий, чтобы все дочерние узлы успели распарситься.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Документ еще парсится: ждем DOMContentLoaded и собираем все section за один проход
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Документ уже распарсен (script внизу body или defer): достаточно собрать на следующем кадре
    ...
    _collectSlides() {
      ...
      this._slides.forEach((slide, idx) => {
        ...
        if (!slide.hasAttribute('data-slide-validate')) {
          slide.setAttribute('data-slide-validate', '');
        }
      });
    }
