...
- Ставь подписи «да» и «нет» на исходящие связи. Внутрь ромба их не помещай.
...
- Ставь события или условия на переходы. Внутрь блоков состояний их не помещай.
...
- Выбирай тип связи до XML: прямая, ломаная, ортогональная, `Curve` или ручной
  маршрут.
- Используй ломаные связи для простых маршрутов с 0-1 изгибом.
- Используй ортогональные связи, когда маршруту нужно 2+ изгиба вокруг
  препятствий.
- Используй `Curve` для причинных карт, концепт-карт и связей, где мягкий
  изгиб помогает отделить смысловые отношения от основного потока.
- Используй ручной маршрут только когда стандартный стиль ухудшает чтение.
  Такой маршрут должен иметь явные точки и визуальный QA.
...
- Авторазводка draw.io запрещена в финальном файле. Если редактор подсказал
  маршрут, перенеси его в явную геометрию и проверь заново.
- `edges[].points` обязательны для ломаных и ортогональных связей, когда
  маршрут меняет ось, обходит препятствие, пересекает пространство дорожки или
  зоны, связывает несоседние узлы либо ведет возвратную петлю по периметру.
- Прямые соседние связи без промежуточных точек допустимы только с явными
  сторонами входа и выхода.
- Кривые и ручные линии не получают автоматическое доверие от скрипта:
  закрывай их предупреждения последним визуальным QA.
...
- Веди длинные обратные связи вокруг внешнего края страницы. Центр основного
  пути чтения не используй.
...
- У связи нет явных промежуточных точек, хотя она меняет ось и пересекает
  границу дорожки.
- Связь оставлена без точек, сторон крепления или другого явного маршрута, а
  вид держится только на авторазводке draw.io.
...
- Кривая связь имеет `curved=1`, явные точки и проверена по последнему экспорту.
...
- Держи каждую страницу читаемой при обычном масштабе. Не рассчитывай на
