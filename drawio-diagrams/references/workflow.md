...
- Авторазводка связей draw.io запрещена в финальном `.drawio`. Черновой маршрут
  из редактора можно использовать только как подсказку для ручного выбора
  геометрии.
- `edges[].points` обязательны для каждого нетривиального ломаного или
  ортогонального маршрута: обхода препятствий, смен оси, пересечений дорожек или
  зон, возвратных петель по периметру и связей между несоседними узлами.
- Прямые соседние связи без промежуточных точек допустимы только с явными
  сторонами входа и выхода либо другой геометрией, которая сохраняется после
  повторного открытия.
- `Curve` и ручные линии требуют явного стиля, явной геометрии и визуального
  QA последнего экспорта.
...
- Классифицируй каждую связь как одну из категорий:
  ...
  - возвратная петля;
  - асинхронный путь.
- Выбери тип линии, наконечник стрелки, сторону входа, сторону выхода и коридор
  для каждой нетривиальной связи.
- Зафиксируй, чем будет доказан маршрут: проверяемой полилинией,
  зафиксированной прямой соседней связью, кривой с визуальным QA или ручной
  геометрией с визуальным QA.
- Длинные возвратные петли веди вокруг внешнего периметра. Середину основной
  области содержимого не используй.
...
- Пересечения связь-связь считай последним вариантом. Пересечения
  связь-вершина — дефекты, компромисс тут не работает.
...
Сборщик печатает предупреждения о маршрутах, когда у связи нет явной геометрии:
точек, зафиксированной прямой соседней связи или другого допустимого маршрута.
Используй `--strict-routes` для сгенерированных диаграмм, которые должны падать
рано, до шлюза компоновки.
...
- связь оставлена на авторазводку draw.io без явного маршрута;
...
- кривая или ручная связь требует визуального QA, потому что скрипт видит только
...
по безопасности. Автоматический обход тут запрещен. Повторяй запуск с
`--allow-desktop-export` только когда пользователь явно принимает риск
...
- связь держится на авторазводке draw.io;
