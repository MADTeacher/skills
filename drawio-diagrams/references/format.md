...
Создавай коннекторы как ячейки-связи. Задавай явную геометрию маршрута, чтобы
путь сохранялся после повторного открытия:
...
- Считай маршрутизацию авторской геометрией. Просмотр draw.io не должен решать
  маршрут.
- Авторазводка draw.io запрещена в финальном исходнике. Связь без явного
  маршрута считается дефектом, даже если редактор временно показывает
  приемлемую линию.
- До записи XML выбери тип линии: `orthogonal`, `elbow`, `straight`, `curve`
  или `manual`.
- `edges[].points` обязательны для любого нетривиального ломаного или
  ортогонального маршрута:
  - обход препятствий;
  - смены оси;
  - пересечения дорожек или зон;
  - связи между несоседними узлами;
  - возвратные петли по периметру.
- Соседние прямые связи могут не иметь промежуточных точек только когда у них
  есть явные `entryX`, `entryY`, `exitX` и `exitY` либо другая зафиксированная
  геометрия, которая сохраняется после повторного открытия.
- Для `Curve` и ручных линий задавай явный стиль и геометрию. Такой маршрут
  требует визуального QA, потому что эвристический шлюз работает с упрощенной
  моделью и не доказывает реальный изгиб.
...
- Прямая связь:
  - `html=1;exitX=1;exitY=0.5;entryX=0;entryY=0.5;endArrow=block;`
- Кривая `Curve` для причинной или концептуальной карты:
  - `edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;curved=1;endArrow=block;`
- Ручной маршрут с зафиксированными точками:
  - `edgeStyle=none;html=1;endArrow=block;`

Поля JSON-спецификации для `scripts/build_drawio.py`:

- `route`: `orthogonal`, `elbow`, `straight`, `curve` или `manual`;
- `points`: массив явных точек маршрута для ломаных, ортогональных, кривых и
  ручных связей;
- `entryX`, `entryY`, `exitX`, `exitY`: стороны крепления к источнику и цели;
- `endArrow`, `startArrow`, `dashed`, `strokeWidth`, `strokeColor`: вид стрелки
  и линии;
- `style`: полный стиль draw.io, если нужна точная ручная настройка.
...
- Оставляй `relative="1"` на геометрии связи, если нет конкретной причины
  сделать иначе.
- Считай маршрутизацию частью авторской геометрии. Просмотр draw.io не должен
  решать маршрут.
- Кодируй задуманный путь явными промежуточными точками, сторонами крепления и
  другими нужными полями геометрии, чтобы повторное открытие файла не
  перестроило связь.
- Если маршрут все равно меняется после повторного открытия, геометрия задана
  недостаточно точно и ее нужно усилить.
- Если ломаный или ортогональный маршрут нетривиален, отсутствие
  `Array as="points"` — дефект авторской геометрии, даже когда draw.io случайно
  отображает правдоподобный путь.
- Если кривая или ручная связь не имеет явной геометрии, это тоже дефект. Не
  используй `Curve` как способ спрятать автоматическую разводку.
...
- промежуточные точки выносят возвратную петлю в коридор периметра; центр не
  используется;
...
- Экранируй пользовательский текст обычной XML-сериализацией. Не собирай
...
- Путь связи меняется после повторного открытия:
  - добавь явные промежуточные точки, стороны крепления или другую геометрию
    связи. Авторазводка draw.io в финальном исходнике запрещена.
...
- Убедись, что маршрутизация коннекторов закодирована в геометрии и не
...
- Для нетривиальных маршрутов добавляй явную геометрию промежуточных точек и
  ...
  правильный коридор.
- Для кривых и ручных связей добавляй явный стиль, явные точки маршрута и
  визуальный QA последнего экспорта.
